...
def calculate_water_level(frame, lower_color, upper_color):
    ...
    # For transparent/white bottles the signal is usually weak/noisy; clean up the mask a bit
    ...
    if contours:
        ...
        water_level_percentage = (water_area / max(total_area, 1)) * 100
...
# Initial lower and upper bounds for color in HSV (transparent/white bottle friendly)
# Idea: water region tends to have LOW saturation and slightly LOWER value than highlights/white plastic.
lower_color = np.array([0, 0, 0])
upper_color = np.array([179, 70, 210])

for i, color_name in enumerate(['Hue', 'Saturation', 'Value']):
    max_val = 179 if color_name == "Hue" else 255
    cv2.createTrackbar(
        f'Lower {color_name}',
        'Water Level',
        int(lower_color[i]),
        max_val,
        lambda val, i=i: on_lower_color_change(val, i),
    )
    cv2.createTrackbar(
        f'Upper {color_name}',
        'Water Level',
        int(upper_color[i]),
        max_val,
        lambda val, i=i: on_upper_color_change(val, i),
    )
